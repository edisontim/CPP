...
int main()
{
	...
	another_form = random_intern.makeForm("PRESIDENTIAL PARDON REQUEST", "Bender");
	Bureaucrat Frank("Frank", 50);
	std::cout << *another_form << std::endl;
	try
	{
		another_form->beSigned(Frank);
		another_form->execute(Frank);
	}
	catch (std::exception &e)
	{
		std::cout << e.what() << std::endl;
	...
	try
	{
		...
		another_form->beSigned(boss);
		another_form->execute(boss);
	}
	catch (std::exception &e)
	{
		std::cout << e.what() << std::endl;
